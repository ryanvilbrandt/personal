...
def SendZRequest(port, msg_to_send, timeout=2, tries=3,
                  my_id='', ret_packet=''):
    '''
    ...
    while tries > 0:
        ...
        try:
            ...
            if DEBUG: print "Packet sent: {0!r}".format(msg_to_send)
...
def ParseZRequest(ptype, payload):
    ...
    if ptype == PTYPE_ZCON:
        ...
        return {'status_mask': payload[0:4],
                ...
                'extra_data_version': myunpack(payload[62:64]),
                'unsent_log_point_count': myunpack(payload[64:68]),
                'unsent_gendata_count': myunpack(payload[68:70]),
                'unsent_tag_event_count': myunpack(payload[70:72]),
                'gps_log_point_timestamp': myunpack(payload[72:76]),
                'latitude': myunpack(payload[76:80], signed=True),
                'longitude': myunpack(payload[80:84], signed=True)
                # To be completed
                }
    else:
        if DEBUG: print "Unrecognized packet type when parsing payload"
...
def ReadMessage(port, my_id=''):
    ...
    if line == '\x02' and port.read(1) == PACKET_VERSION:
        ...
        if DEBUG: print "Packet header received: {0!r}".format(packet)
        ...
        if DEBUG: print "Packet received: {0!r}".format(packet)
...
def CreateSetConfValMessage(conf, val,
    ...
    @return str -- Valid ZMessage
    '''
    body = struct.pack("<H", conf) + val
    return CreateMessage(ptype, source, target, body)

def CreateFwUpdateMessage(stage, x_of, limit, payload="",
                          ptype=PTYPE_FW_UPDATE_T, source=DEVICE_PC,
                          target=DEVICE_ZCONMDR):
    '''
...
def GetStatusInfo(port, tries=6, timeout=2):
    '''
...
def SetConfVal(port, conf, val):
    '''
    Sets a conf value on the attached V3
    @return str -- Response from device or None
    '''
    return SendZRequest(port, CreateSetConfValMessage(conf, val),
